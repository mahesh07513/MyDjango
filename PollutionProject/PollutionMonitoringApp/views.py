...
from PollutionMonitoringApp.models import *
   ...
   #{
	...
	print request.body
     ...
     #{
          ...
          output = '{"error_code":"0", "Response":"Successfully Authenticated for user: %s ","Role":"user"}' %(username)
          logging.debug("login:"+ output)
          return HttpResponse(output)

     else:
        if(username == "jtsadmin") or (password == "jtsadmin"):
           output = '{"error_code":"0", "Response":"Successfully Authenticated for user: %s ","Role":"admin"}' %(username)
           logging.debug("login:"+ output)
           return HttpResponse(output)
        else:
           output = '{"error_code":"2", "error_desc": "Response=%s"}' %output_str
   ...
   #{
	...
	# load done
	   url = ''
        ...
        curr_status_rec = Current_Status(TowerId=towerid, Status=status,ChangeBy=username,Operate=operate);
        ...
        #{
           ...
           #{
              curr_status_hist_rec = Current_Status_History(TowerId=towerid, Status=status,ChangeBy=username);
   ...
   #{
     ...
     #{
        ...
        #{ 
           ...
           #{
              ...
              #{
                 counter += 1
                 if(counter == 1):
                   output += '{"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"00:00","EndTime":"23:00"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate)
                 else: 
                   output += ',\n {"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"00:00","EndTime":"23:00"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate)
              #}
              output += ']\n'
              #output += '}'
              #cust_recs_json = serializers.serialize("json", curr_status_recs)
        ...
        #{
           ...
           today1 = dt1.date.today();
         
           
           end_date = datetime.strftime(kwargs['ChangeDate__gte'], '%Y-%m-%d')
           print(end_date)
           curr_status_recs = Current_Status_History.objects.filter(ChangeBy=username,**kwargs)
           if(len(output)<= 10): 
              output = '{"error_code":"0", "error_desc": "Response=Current_Status history recs"' 
           ...
           #{
              ...
              #{
                 counter += 1
                 if(counter == 1):
                   output += '{"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,end_date,today1)
                 else: 
                   output += ',\n {"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,end_date,today1)
              #}
              output += ']\n'
           #}
           else:
	   ...
	   print(kwargs)
           ...
           #{
              ...
              #{
                 counter += 1
                 if(counter == 1):
                   output += '{"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,month_old_date,today)
                 else: 
                   output += ',\n {"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,month_old_date,today)
              #}
              output += ']\n'
           #}
           else:
        ...
        #{
           ...
           #{
              ...
              #{
                 counter += 1
                 if(counter == 1):
                   output += '{"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,year_old_date,today)
                 else: 
                   output += ',\n {"ActivityId":"%s","TowerId":"%s", "Status":"%s", "ChangeDate":"%s","StartTime":"%s","EndTime":"%s"}' %(rec.ActivityId, rec.TowerId, rec.Status, rec.ChangeDate,year_old_date,today)
              #}
              output += ']\n'
           #}
           else:
...
def get_all_ids(request):
   ...
   if(request.method == "POST"):
   #{
     logging.debug("get_all_ids: request is from the IP:%s" %request.META.get('REMOTE_ADDR'))
     output_str = "get_all_ids- Unable to Authenticate/get_all_ids... " 
     try:
        data1 = json.loads(request.body)
     except ValueError:
        output_str += ",invalid input, no proper JSON request "
        output = '{"error_code":"2", "error_desc": "Response=%s"}' %(output_str )
        logging.debug("get_all_ids:"+ output)
        return HttpResponse(output)
     ...
     username    = data1['username']
     password    = data1['password']
     logging.debug("get_all_ids:input: user="+username)
     
     #check authentication (temporary, this will need to be implemented) 
     if(username != "jtsadmin") or (password != "jtsadmin"):
        output = '{"error_code":"2", "error_desc": "Response=%s"}' %output_str
        logging.debug("get_all_ids:Failed to Authenticate"+ output)
        return HttpResponse(output)
     ...
     #{
        ...
        #Get the Curent_Status records
        print("aray data",kwargs)
        curr_status_recs = Current_Status.objects.filter(**kwargs)
        if(len(curr_status_recs) > 0): 
        #{
           print("in")
           output = '{"error_code":"0", "error_desc": "Response=Successfully got %d Current_Status recs", \n "current_ids":' %len(curr_status_recs)
           output += '['
           counter = 0
           for rec in curr_status_recs:
           #{
              counter += 1
              if(counter == 1):
                output += '{"TowerId":"%s", "UserName":"%s"}' %(rec.TowerId, rec.ChangeBy)
              else: 
                output += ',\n {"TowerId":"%s", "UserName":"%s"}' %(rec.TowerId, rec.ChangeBy)
           #}
           output += ']\n'
           output += '}'
           #cust_recs_json = serializers.serialize("json", curr_status_recs)
        ...
        #}
           ...
           output = '{"error_code":"3", "error_desc": "Response=Failed to get the machines records, NO_DATA_FOUND"}' 
           logging.debug("get_all_ids:"+ output)
           return HttpResponse(output)
     #}
     except Exception, e:
     #{
         err_desc = 'get_all_ids:exception details:[%s],[%s]' %((sys.exc_info()[0]), (sys.exc_info()[1]))
         logging.debug("get_all_ids:"+ err_desc)
         output = '{"error_code":"3", "error_desc": "Response=Failed to get the machines"}' 
         logging.debug("get_all_ids:"+ output)
         return HttpResponse(output)
     #}
   #}
